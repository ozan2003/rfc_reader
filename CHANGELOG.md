...
- Use `rustls` as the TLS provider for `ureq` in [client.rs](src/client.rs).

## [0.13.6] - 2026-07-25

## Changed

- Split [app.rs](src/ui/app.rs) into multiple modules for better organization and maintainability.
- The splitted modules are:
  - [render.rs](src/ui/render.rs): Contains all UI drawing code including the main document view, help overlay, search box, statusbar, and too-small message.
  - [search.rs](src/ui/search.rs): Contains the search logic and parallel/serial search implementations.

## [0.13.5] - 2026-07-25

## Changed

- Use `log::error!` instead of `eprintln!` for error logging in [event.rs](src/ui/event.rs)

## [0.13.4] - 2026-04-03

## Fixed

- Fixed a bug where search mode checkboxes were clipped in the statusbar in [app.rs](src/ui/app.rs)

## Changed

- Refactored the statusbar layout logic to size sections to their actual content in [app.rs](src/ui/app.rs)

## [0.13.3] - 2026-03-29

## Changed

- Refactored the document progress calculation logic to handle empty documents
and 100% progress in [app.rs](src/ui/app.rs)

## [0.13.2] - 2026-03-28

## Added

- Introduced a new `SearchStrategy` enum to encapsulate the search strategy in [app.rs](src/ui/app.rs)
for better type safety.

## Changed

- Refactored whether the search should be done in parallel manner in [app.rs](src/ui/app.rs).
...
- Text search now runs in parallel for large documents in
[app.rs](src/ui/app.rs), improving performance.
...
- Refactored event handling logic to redraw only on relevant events in
...
- Added a new test for the cache in [cache.rs](src/cache.rs)

## Changed

- Use `Box<Path>` instead of `PathBuf` in [cache.rs](src/cache.rs)
- Use `context` for cache initialization in [main.rs](src/main.rs)
...
- Change wording from "log file" to "log files" in [logging.rs](src/ui/logging.rs) and
[main.rs](src/main.rs)
...
- Added a log file rotation mechanism with compression in [logging.rs](src/ui/logging.rs)

## [0.8.2] - 2025-11-06

## Changed

- Use `Box<Path>` instead of `PathBuf` in [logging.rs](src/ui/logging.rs)

## [0.8.1] - 2025-10-29

## Added

- Introduce an accessor function instead of directly using the log file path in
[logging.rs](src/ui/logging.rs)
...
- Added support for case-sensitive and regex searches in [app.rs](src/ui/app.rs) and
[main.rs](src/main.rs)

## Changed

- Regexes are now cached to improve performance in [app.rs](src/ui/app.rs)
...
- Introduced new lints in [Cargo.toml](Cargo.toml) and applied them in
[app.rs](src/ui/app.rs), [guard.rs](src/guard.rs) and [toc_panel.rs](src/ui/toc_panel.rs)
